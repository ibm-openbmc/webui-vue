...
export const HardwareDeconfigurationStore = defineStore(
          ...
          api.spread((...responses) => {
            const coreData = responses.map(({ data }) => {
              ...
              if (Array.isArray(conditionsArray) && conditionsArray.length) {
                ...
                if (
                  Array.isArray(messageArgsArray) &&
                  messageArgsArray.length
                ) {
      ...
      async updateSettingsState(settingsState) {
        ...
        return await api.patch(uri, updatedSettingsValue).catch((error) => {
          ...
          if (REGEX_MAPPINGS.resourceCannotBeDeleted.test(messageId)) {
            ...
            throw new Error(
              i18n.global.t(
                'pageDeconfigurationHardware.toast.errorEnablingSetting',
              ),
            );
          } else {
            throw new Error(
              i18n.global.t(
                'pageDeconfigurationHardware.toast.errorDisablingSetting',
              ),
            );
          }
        });
