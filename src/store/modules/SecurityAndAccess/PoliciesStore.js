...
export const PoliciesStore = defineStore('policies', {
  ...
  getters: {
    ...
    getUsbFirmwareUpdatePolicyEnable: (state) => state.usbFirmwareUpdatePolicyEnabled,
  ...
  actions: {
    ...
    async saveTpmPolicy(protocolEnabled) {
      this.tpmPolicyEnabled = protocolEnabled.isTrusted;
      const data = {
        Boot: {
          TrustedModuleRequiredToBoot: protocolEnabled.isTrusted,
        },
      };
      // TODO: remove hardcoded endpoint when fix is available
      return api
    ...
    async saveUsbFirmwareUpdatePolicyEnabled(updatedUsbCode) {
      this.usbFirmwareUpdatePolicyEnabled = updatedUsbCode.isTrusted;
      const oem = {
        Oem: {
          IBM: {
            USBCodeUpdateEnabled: updatedUsbCode.isTrusted,
          },
        },
      };
      return await api
    ...
    async saveIpmiProtocolState(protocolEnabled) {
      this.ipmiProtocolEnabled = protocolEnabled.isTrusted;
      const ipmi = {
        IPMI: {
          ProtocolEnabled: protocolEnabled.isTrusted,
        },
      };
      return await api
    ...
    async saveSshProtocolState(protocolEnabled) {
      this.sshProtocolEnabled = protocolEnabled.isTrusted;
      const ssh = {
        SSH: {
          ProtocolEnabled: protocolEnabled.isTrusted,
        },
    ...
    async saveRtadState(updatedRtad) {
      this.setRtadEnabled = updatedRtad.isTrusted;
      return await api
        .patch('/redfish/v1/Systems/system/Bios/Settings', {
          Attributes: {
            pvm_rtad: updatedRtad.isTrusted === 'true' ? 'Enabled' : 'Disabled',
          },
        })
        .then(() => {
    ...
    async saveVtpmState(updatedVtpm) {
      this.vtpmEnabled = updatedVtpm.isTrusted;
      return await api
        .patch('/redfish/v1/Systems/system/Bios/Settings', {
          Attributes: {
            pvm_vtpm: updatedVtpm.isTrusted === 'true' ? 'Enabled' : 'Disabled',
          },
        })
        .then(() => {
          return i18n.global.t('pagePolicies.toast.successNetworkPolicyUpdate', {
    ...
    async saveSvleState(updatedSvle) {
      this.svleEnabled = updatedSvle.isTrusted;
      return await api
        .patch('/redfish/v1/Systems/system/Bios/Settings', {
          Attributes: {
            hb_secure_ver_lockin_enabled: updatedSvle.isTrusted === 'true' ? 'Enabled' : 'Disabled',
          },
        })
        .then(() => {
          return i18n.global.t('pagePolicies.toast.successNetworkPolicyUpdate', {
    ...
    async saveHostUsbEnabled(updatedHostUsb) {
      this.setHostUsbEnabled = updatedHostUsb.isTrusted;
      return await api
        .patch('/redfish/v1/Systems/system/Bios/Settings', {
          Attributes: {
            hb_host_usb_enablement: updatedHostUsb.isTrusted === 'true' ? 'Enabled' : 'Disabled',
          },
        })
        .then(() => {
          return i18n.global.t('pagePolicies.toast.successNextBootToast', {
