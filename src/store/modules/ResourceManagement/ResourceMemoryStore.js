...
const ResourceMemoryStore = {
  ...
  getters: {
    ...
    dynamicIoDrawerDefaultCapacity: (state) => state.dynamicIoDrawerDefaultCapacity,
  ...
  mutations: {
    ...
    setDynamicIoDrawerDefaultCapacity: (state, dynamicIoDrawerDefaultCapacity) =>
    ...
    setMaxNumHugePages: (state, maxNumHugePages) => (state.maxNumHugePages = maxNumHugePages),
    setNumHugePages: (state, numHugePages) => (state.numHugePages = numHugePages),
  ...
  actions: {
    async getMemorySizeOptions({ commit }) {
      return await api
        ...
        .then(({ data: { RegistryEntries } }) => {
          ...
          let memorySizeOptions = memorySize[0].Value.map(({ ValueName }) => ValueName);
      ...
      return await api
        ...
        .then(({ data: { RegistryEntries } }) => {
          const ioAdapterCapacity = RegistryEntries.Attributes.filter(
            (Attribute) => Attribute.AttributeName == 'hb_ioadapter_enlarged_capacity',
          );
          ...
          const dynamicIoDrawerCapacity = RegistryEntries.Attributes.filter(
            (Attribute) => Attribute.AttributeName == 'hb_storage_preallocation_for_drawer_attach',
          );
          let dynamicIoDrawerAttachmentCapacity = dynamicIoDrawerCapacity[0].CurrentValue;
          commit('setDynamicIoDrawerCapacity', dynamicIoDrawerAttachmentCapacity);
          ...
          commit('setDynamicIoDrawerDefaultCapacity', dynamicIoDrawerAttachmentDefaultCapacity);
        })
        .catch((error) => console.log(error));
    },
      ...
      return await api
        ...
        .then(({ data: { RegistryEntries } }) => {
          ...
          if (activeMemoryMirroringMode.length > 0) {
            let activeMemoryMirroringModeValue = activeMemoryMirroringMode[0].CurrentValue;
            let mirroringModeValue = activeMemoryMirroringModeValue == 'Enabled' ? true : false;
            commit('setMemoryMirroringMode', mirroringModeValue);
          }
        })
        .catch((error) => console.log(error));
    },
    async saveActiveMemoryMirroringMode({ commit }, activeMemoryMirroringModeValue) {
      let updatedMirroringModeValue = activeMemoryMirroringModeValue ? 'Enabled' : 'Disabled';
      ...
      return api
        ...
        .then(() => {
          return i18n.t('pageMemory.toast.successSavingActiveMemoryMirroringMode');
        })
        .catch((error) => {
          console.log(error);
          commit('setMemoryMirroringMode', !activeMemoryMirroringModeValue);
          throw new Error(i18n.t('pageMemory.toast.errorSavingActiveMemoryMirroringMode'));
        });
    },
    async savePageSetup({ commit }) {
      ...
      return await api
        ...
        .then(() => {
          commit('setNumHugePages', updatedNumHugePages.Attributes.hb_number_huge_pages);
    ...
    async saveEnlargedCapacity({ commit }) {
      const updatedIoEnlargedCapacity = {
        Attributes: {
          hb_ioadapter_enlarged_capacity: this.state.resourceMemory.ioAdapterCapacity,
        },
      };
      return await api
        ...
        .then(() => {
          ...
          return i18n.t('pageMemory.toast.successSavingAdapterEnlargedCapacity');
        })
        .catch((error) => {
          console.log('error', error);
          throw new Error(i18n.t('pageMemory.toast.errorSavingAdapterEnlargedCapacity'));
        });
    },
    async saveDynamicCapacity({ commit }) {
      ...
      return await api
        .patch('/redfish/v1/Systems/system/Bios/Settings', updatedIoDynamicCapacity)
        .then(() => {
          commit(
        ...
        .catch((error) => {
          console.log(error);
          throw new Error(i18n.t('pageMemory.toast.errorSavingAdapterDynamicCapacity'));
        });
    },
    async saveSettings({ commit }, logicalMemorySize) {
      ...
      return await api
        ...
        .then(() => {
          commit('setLogicalMemorySize', updatedMemorySize.Attributes.hb_memory_region_size);
