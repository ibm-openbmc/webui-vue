...
export function useLdap() {
    ...
    const isServiceEnabled = computed(() => {
        const ldapEnabled = accountServiceQuery.data.value?.LDAP?.ServiceEnabled ?? false;
        const adEnabled = accountServiceQuery.data.value?.ActiveDirectory?.ServiceEnabled ?? false;
        return ldapEnabled || adEnabled;
    });

    const isActiveDirectoryEnabled = computed(() => {
        return accountServiceQuery.data.value?.ActiveDirectory?.ServiceEnabled ?? false;
    });
    ...
    const ldapSettings = computed<LdapSettings>(() => {
        ...
        return {
            ...
            baseDn: ldapData?.LDAPService?.SearchSettings?.BaseDistinguishedNames?.[0] ?? '',
    ...
    const activeDirectorySettings = computed<LdapSettings>(() => {
        ...
        return {
            ...
            baseDn: adData?.LDAPService?.SearchSettings?.BaseDistinguishedNames?.[0] ?? '',
    ...
    const saveLdapMutation = useMutation({
        mutationFn: async (properties: Partial<LdapService>): Promise<string> => {
    ...
    async function saveAccountSettings(params: SaveAccountSettingsParams): Promise<string> {
        ...
        if (params.activeDirectoryEnabled) {
            return await saveActiveDirectoryMutation.mutateAsync(data);
        } else {
            return await saveLdapMutation.mutateAsync(data);
        }
    }
    ...
    const addRoleGroupMutation = useMutation({
        mutationFn: async ({
            ...
            await api.patch('/redfish/v1/AccountService', data);
            return i18n.global.t('pageLdap.toast.successAddRoleGroup', { groupName });
        },
        onSuccess: () => {
            queryClient.invalidateQueries({
                queryKey: ['redfish', 'resource', '/redfish/v1/AccountService'],
    ...
    const saveRoleGroupMutation = useMutation({
        mutationFn: async ({
            ...
            await api.patch('/redfish/v1/AccountService', data);
            return i18n.global.t('pageLdap.toast.successSaveRoleGroup', { groupName });
        },
        onSuccess: () => {
            queryClient.invalidateQueries({
                queryKey: ['redfish', 'resource', '/redfish/v1/AccountService'],
    ...
    async function addNewRoleGroup(params: {
        ...
        return await addRoleGroupMutation.mutateAsync(params);
    }
    ...
    async function saveRoleGroup(params: {
        ...
        return await saveRoleGroupMutation.mutateAsync(params);
    }

    async function deleteRoleGroup(params: {
        roleGroups: Array<{ groupName: string }>;
    }): Promise<string> {
        return await deleteRoleGroupMutation.mutateAsync(params);
    }
